...
export default function StrikeStatusDemo({ studentId }) {
          ...
          {history.map((strike, index) => (
            <div key={strike.id} className="strike-detail-item">
              <div className="strike-detail-header">
                <span className="strike-number">Strike #{strike.strikeNumber}</span>
              ...
              <p className="strike-meta">
                Equipment was {strike.daysOverdue} day(s) overdue
                {strike.restrictionDays > 0 && ` • ${strike.restrictionDays} day restriction applied`}
              </p>
            </div>
          ))}
